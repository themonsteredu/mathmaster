...
const COST = { clean: 55, unit: 0.3, type: 3 };
...
export default function UsagePage() {
  ...
  const mType = count(thisMonth, "type");
  const mCost = mClean * COST.clean + mUnit * COST.unit + mType * COST.type;

  const tClean = count(rows, "clean");
  const tUnit = count(rows, "unit");
  const tType = count(rows, "type");
  const tCost = tClean * COST.clean + tUnit * COST.unit + tType * COST.type;

  // 월별 집계
  const byMonth = new Map<string, { clean: number; unit: number; type: number }>();
  rows.forEach((r) => {
    const m = r.created_at.slice(0, 7);
    const cur = byMonth.get(m) ?? { clean: 0, unit: 0, type: 0 };
    if (r.kind === "clean") cur.clean++;
    else if (r.kind === "unit") cur.unit++;
    else if (r.kind === "type") cur.type++;
    byMonth.set(m, cur);
  });
  ...
  return (
    <>
      <PageHeader eyebrow="계정" title="API 사용량" sub="AI 기능(낙서 지우기·단원 인식·유형 분류) 호출 횟수와 예상 비용이에요." />
      ...
      <div className="grid grid-3">
        ...
        <StatCard label="유형 분류" value={mType} unit="회" hint={`약 ${won(mType * COST.type)}`} />
      ...
      <div className="grid grid-3">
        ...
        <StatCard label="유형 분류" value={tType} unit="회" hint={`약 ${won(tType * COST.type)}`} />
        ...
        <>
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <table className="table">
              <thead>
                <tr><th>월</th><th>낙서 지우기</th><th>단원 인식</th><th>유형 분류</th><th>예상 비용</th></tr>
              </thead>
              <tbody>
                {months.map(([m, c]) => (
                  <tr key={m}>
                    ...
                    <td><span className="num">{c.type}</span>회</td>
                    <td><span className="num">{won(c.clean * COST.clean + c.unit * COST.unit + c.type * COST.type)}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="alert alert-info" style={{ marginTop: 18 }}>
        💡 위 금액은 <b>추정치</b>예요 (낙서 지우기 약 {COST.clean}원/회, 단원 인식 약 {COST.unit}원/회, 유형 분류 약 {COST.type}원/회로 계산).
        낙서 지우기·단원 인식은 <b>Google AI Studio</b>, 유형 분류는 <b>Anthropic(Claude)</b>에서 실제 청구·무료 한도를 확인하세요.
      </div>
    </>
  );
}
